...
param HEAT_GEN_COST{i in I} := (1 / beta) * T_flh[source] * c_heat[source]; /* * sum Pin of source to the others*/

param MAINT_COST{i in I, j in J} := c_om[i, j] * l[i, j] * x[i, j];  

param FIXED_INV_COST{i in I, j in J} := alpha * x[i, j] * c_fix[i, j] * l[i, j]; 

param VAR_INV_COST{i in I, j in J} := alpha * c_var[i, j] * l[i, j]; /* by the amount of flow */
...
param l{i in I, j in J}:= sqrt((node_coord_x[i, j] * node_coord_x[i, j]) + node_coord_y[i, j] * node_coord_y[i, j])
...
s.t. unidirectionality{i in I, j in J: i<>j}: x[i, j] + x[j, i] <= 1; /* i != j */
...
s.t. flow_equilibrium_at_each_vertex{j in J: j <> source}: sum{i in I} P_in[i, j] = sum{i in J} P_out[i, j];/* Pin i->j == P j->k */  /* HOW TO */

s.t. edge_capacity{i in I, j in J, i<>j}: P_in[i, j] <= C_max[i, j] * x[i, j];

s.t. source_structural{j in J}: sum{i in I} x[i, source] = 0;

s.t. source_heat_generation: sum{j in J} P_in[source, j] <= Q_max; /* Pin ij <= Q max (it should be just invertices, even an unique parameter*/ 

s.t. tour_elimination{i in I}: sum{j in J, j <> i} x[j, i] >= 1;
  

/* DATA */
/* what is vvar and vfix?'?? */
data;
param source := 4;
param node_coord_x :=
	1	10
