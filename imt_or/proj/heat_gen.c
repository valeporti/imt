...
/* sub expressions */
param l{i in I, j in J: i<>j} := sqrt((node_coord_x[i] - node_coord_x[j]) * (node_coord_x[i] - node_coord_x[j]) + (node_coord_y[i] - node_coord_y[j]) * (node_coord_y[i] - node_coord_y[j])); /* longueur */
